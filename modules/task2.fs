    ...
    let rec generateCells n =
        seq {
            if n > 1L then
                ...
                for i in 1L .. n - 1L do
                    yield { x = (n-1L)/2L; y = (n-1L)/2L-i; num = n * n - 4L * n + 4L + i }
                for i in 1L .. n - 1L do
                    yield { x = (n-1L)/2L-i; y = -(n-1L)/2L; num = n * n - 4L * n + 4L + n-1L + i }
                for i in 1L .. n - 1L do
                    yield { x = -(n-1L)/2L; y = -(n-1L)/2L+i; num = n * n - 4L * n + 4L + (n-1L) * 2L+ i }
                for i in 1L .. n - 1L do
                    yield { x = -(n-1L)/2L+i; y = (n-1L)/2L; num = n * n - 4L * n + 4L + (n-1L) * 3L + i }
            else yield {x = 0L;y = 0L;num = 1L}
        }
